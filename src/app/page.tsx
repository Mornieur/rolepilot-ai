...
export default async function Home() {
  ...
  try { jobStatusCountsByProfile = Object.fromEntries(await Promise.all(profileResult.profiles.map(async (profile) => [profile.id, await listStatusCountsByProfile(profile.id)]))); } catch { jobStatusCountsError = "Please try again."; }
  return <Dashboard profiles={profileResult.profiles} jobs={jobs} analyses={jobAnalyses} companies={companyResult.companies ?? []} companyError={companyResult.error} persistedJobCount={persistedJobs.jobs?.length ?? 0} jobStatusCountsByProfile={jobStatusCountsByProfile} jobStatusCountsError={jobStatusCountsError} />;
}
