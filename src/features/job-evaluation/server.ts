...
export async function evaluatePersistedJobsForProfile(profileId: string): Promise<DeterministicJobEvaluation[]> {
  const profile = await getCandidateProfileById(profileId);
  if (!profile) throw new Error("The selected profile could not be found.");

  return sortEvaluations((await listPersistedJobs()).map((persistedJob) => evaluateJob(profile, persistedJob)));
}
