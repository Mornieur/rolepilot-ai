...
import type {
  ...
  EvaluationReason,
} from "@/features/job-evaluation/types";
...
function detectModels(text: string, location: string | null, offices: string[]): WorkModel[] {
  ...
  if (
    /\b(remote|remoto)\s+(role|position|work|trabalho|opportunity|job|vaga)\b|\b(fully|100)\s+(remote|remoto)\b|\b(work|working)\s+(remote|remotely)\b/.test(text) ||
    /\b(remote|remoto)\b/.test(locationText)
  ) result.push("remote");
  if (/\b(hybrid|hibrido)\b/.test(text) || /\b(hybrid|hibrido)\b/.test(locationText)) result.push("hybrid");
  if (/\b(on site|onsite|presencial)\b/.test(text) || /\b(on site|onsite|presencial)\b/.test(locationText)) result.push("on-site");

  return result;
}

export function evaluateJob(profile: CandidateProfile, job: PersistedJob): DeterministicJobEvaluation {
  ...
  const requiredOk = profile.requiredSkills.length === 0 || required.length === unique(profile.requiredSkills).length;
  const titleOk = profile.desiredRoles.length === 0 || titleTerms.length > 0;
  const excludedOk = excluded.length === 0;
  const seniorityOk = detectedSeniorities.length === 0
    ? null
    : detectedSeniorities.some((level) => profile.acceptedSeniorities.includes(level));
  const locationOk = !job.location || profile.locations.length === 0 ? null : locationTerms.length > 0;
  const workModelOk = detectedModels.length === 0 || profile.acceptedWorkModels.length === 0
    ? null
  ...
  } else reasons.push({ code: "required", outcome: "fail", message: "One or more required keywords are missing." });
  if (preferred.length) {
  ...
  for (const [code, value, label] of [
    ...
    } else reasons.push({ code, outcome: "neutral", message: `${label} is unknown or unrestricted.` });
  }

  const eligible = requiredOk && titleOk && excludedOk && seniorityOk !== false && locationOk !== false && workModelOk !== false;
  return {
    job,
    ...
    titleMatch: { matched: profile.desiredRoles.length ? titleTerms.length > 0 : null, matchedTerms: titleTerms },
