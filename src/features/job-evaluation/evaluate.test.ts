...
function job(overrides: Partial<PersistedJob> = {}): PersistedJob {
  return {
    id: "job-1", provider: "greenhouse", targetCompanyId: "company-1", externalId: "external-1",
    title: "Senior Frontend Engineer", location: "Sao Paulo, Brazil",
    descriptionText: "Remote role using React, TypeScript, GraphQL, and C#.", originalUrl: "https://example.test/jobs/1",
    sourceUpdatedAt: null, language: "en", departments: ["Engineering"], offices: [],
    firstSeenAt: "2026-07-01T00:00:00.000Z", lastSeenAt: "2026-07-02T00:00:00.000Z",
...
describe("deterministic job evaluator", () => {
  ...
  it("rejects missing required terms even when other terms match", () => {
    ...
    expect(result.reasons).toContainEqual(expect.objectContaining({ code: "required", outcome: "fail" }));
  });

  it("rejects an excluded exact keyword without treating JavaScript as Java", () => {
    expect(evaluateJob(profile, job({ descriptionText: "Remote role using React, TypeScript, and JavaScript." })).eligible).toBe(true);
    const result = evaluateJob(profile, job({ descriptionText: "Remote role using React, TypeScript, and Java." }));
    expect(result.eligible).toBe(false);
  ...
  it("uses title-only seniority detection, including Portuguese terms, and treats unknown as neutral", () => {
    expect(evaluateJob({ ...profile, acceptedSeniorities: ["mid"] }, job({ title: "Desenvolvedor Pleno Frontend Engineer" })).eligible).toBe(true);
    const unknown = evaluateJob(profile, job({ title: "Frontend Engineer", descriptionText: "Senior candidates welcome. Remote role using React and TypeScript." }));
  ...
  it("rejects an explicit incompatible location and work model, but keeps missing values neutral", () => {
    expect(evaluateJob(profile, job({ location: "New York", descriptionText: "Remote role using React and TypeScript." })).eligible).toBe(false);
    expect(evaluateJob(profile, job({ location: "Sao Paulo", descriptionText: "Onsite position using React and TypeScript." })).eligible).toBe(false);
    const neutral = evaluateJob(profile, job({ location: null, descriptionText: "React and TypeScript are used." }));
  ...
  it("does not mistake a remote-team reference for an explicit work model", () => {
    const result = evaluateJob(profile, job({ descriptionText: "Work with remote teams using React and TypeScript." }));
  ...
  it("sorts eligible results, then score, freshness, title, and id deterministically", () => {
    ...
    const sameScoreOlder = { ...eligible, job: { ...eligible.job, id: "older", lastSeenAt: "2026-07-01T00:00:00.000Z" } };
