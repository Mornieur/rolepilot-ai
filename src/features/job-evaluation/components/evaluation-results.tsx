...
export function EvaluationResults({
  ...
  return (
    <section className="mt-8" aria-live="polite">
      ...
      {results.length === 0 ? (
        <p className="mt-4">No persisted jobs to evaluate.</p>
      ) : (
        <>
          <div className="mt-4 flex flex-wrap gap-2" aria-label="Filter evaluation results">
            {(['all', 'eligible', 'rejected'] as const).map((value) => (
              <button
                ...
                className="rounded border border-slate-300 bg-white px-3 py-1 text-sm capitalize"
              >
                {value}
              </button>
            ))}
          ...
          <div className="mt-4 space-y-4">
            {visible.map((result) => (
              <article key={result.job.id} className="rounded border border-slate-200 bg-white p-4">
                ...
                {result.eligible ? (
                  ...
                  <p className="mt-3 text-sm text-slate-600">
                    AI analysis is available only after deterministic eligibility passes.
                  </p>
                )}
                <a
