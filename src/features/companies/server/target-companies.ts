...
export class TargetCompanyDataError extends Error {
  constructor(message = targetCompanyDatabaseMessage(false)) { super(message); }
}
...
export async function listTargetCompanies(): Promise<TargetCompany[]> {
  const { data, error } = await getSupabaseServerClient().from("target_companies").select("*").order("created_at");
...
export async function getTargetCompanyById(id: string): Promise<TargetCompany | null> {
  const { data, error } = await getSupabaseServerClient().from("target_companies").select("*").eq("id", id).maybeSingle();
...
export async function createTargetCompany(input: TargetCompanyInput): Promise<TargetCompany> {
  const { data, error } = await getSupabaseServerClient().from("target_companies").insert(toTargetCompanyInsert(input)).select().single();
  if (error || !data) throwDataError("create", error?.code === "23505");
  return toTargetCompany(data);
}

export async function updateTargetCompany(id: string, input: TargetCompanyInput): Promise<TargetCompany> {
  const { data, error } = await getSupabaseServerClient().from("target_companies").update(toTargetCompanyInsert(input)).eq("id", id).select().single();
...
export async function setTargetCompanyEnabled(id: string, enabled: boolean): Promise<TargetCompany> {
  const { data, error } = await getSupabaseServerClient().from("target_companies").update({ enabled }).eq("id", id).select().single();
