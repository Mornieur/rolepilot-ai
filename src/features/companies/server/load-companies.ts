...
import { getTargetCompanyById, listTargetCompanies, TargetCompanyDataError } from "@/features/companies/server/target-companies";
...
export async function loadTargetCompanyById(id: string): Promise<{ company: TargetCompany | null; error: string | null }> {
  if (!targetCompanyIdSchema.safeParse(id).success) return { company: null, error: "The requested company could not be found." };
  try {
    const company = await getTargetCompanyById(id);
    return company ? { company, error: null } : { company: null, error: "The requested company could not be found." };
  } catch (error) {
    if (error instanceof SupabaseConfigurationError || error instanceof TargetCompanyDataError) return { company: null, error: error.message };
