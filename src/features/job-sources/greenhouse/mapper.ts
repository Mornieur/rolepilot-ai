...
import {
  ...
  type GreenhouseJob,
} from '@/features/job-sources/greenhouse/schemas';
import type {
  ExternalJobPreview,
  GreenhousePreviewResult,
} from '@/features/job-sources/greenhouse/types';

function names(items: { name: string }[] | null | undefined) {
...
export function mapGreenhouseJob(job: GreenhouseJob, company: TargetCompany): ExternalJobPreview {
  return {
    ...
    descriptionText: greenhouseHtmlToText(job.content),
...
export function mapGreenhouseResponse(
  payload: unknown,
  ...
  const jobs = parsedJobs
    ...
    .map((job) => mapGreenhouseJob(job.data, company));
  return {
    jobs,
    total: response.data.meta?.total ?? jobs.length,
    skippedJobs: parsedJobs.length - jobs.length,
  };
}
