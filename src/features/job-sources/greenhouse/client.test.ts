...
const company: TargetCompany = { id: "11111111-1111-4111-8111-111111111111", name: "Example", provider: "greenhouse", boardIdentifier: "Example_Board", enabled: true, priority: "normal", createdAt: "2026-01-01T00:00:00Z", updatedAt: "2026-01-01T00:00:00Z" };

afterEach(() => vi.unstubAllGlobals());

describe("Greenhouse client", () => {
  it("constructs an official HTTPS API URL and rejects unsafe board identifiers", () => {
    expect(greenhouseJobsUrl("Example_Board")).toBe("https://boards-api.greenhouse.io/v1/boards/example_board/jobs?content=true");
  ...
  it("maps a successful no-store connector response", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ jobs: [{ id: 1, title: "Engineer", absolute_url: "https://boards.greenhouse.io/example/jobs/1" }] }), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    await expect(fetchGreenhouseJobs(company)).resolves.toMatchObject({ total: 1, jobs: [{ externalId: "1" }] });
