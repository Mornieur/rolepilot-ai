...
import { aiJobAnalysisSchema, classifyAiAnalysisValidationFailure } from './schema';
...
describe('AI structured-output classifier', () => {
  ...
  it('classifies missing fields, enums, types, nulls, and string constraints', () => {
    ...
    expect(classification({ ...valid, summary: '' })).toBe('string_constraint');
    expect(classification({ ...valid, summary: 'x'.repeat(601) })).toBe('string_constraint');
  });
});
