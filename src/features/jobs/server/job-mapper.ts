...
export function toPersistedJob(row: PersistedJobRow): PersistedJob {
  ...
  return {
    ...
    departments: arrays(row.departments),
    offices: arrays(row.offices),
...
export function previewToJobFields(preview: ExternalJobPreview) {
  return {
    provider: 'greenhouse' as const,
    target_company_id: preview.companyId,
    external_id: preview.externalId,
    title: preview.title,
    location: preview.location,
    description_text: preview.descriptionText,
    original_url: preview.originalUrl,
    source_updated_at: preview.sourceUpdatedAt,
