...
const profiles: CandidateProfile[] = [
  { id: mockProfileIds.frontend, name: "Frontend specialist", desiredRoles: ["Frontend Engineer", "React Engineer"], acceptedSeniorities: ["senior", "staff"], requiredSkills: ["React", "TypeScript"], preferredSkills: [], excludedSkills: [], acceptedWorkModels: ["remote", "hybrid"], locations: ["Brazil"] },
  { id: mockProfileIds.data, name: "Data and BI specialist", desiredRoles: ["Data Analyst"], acceptedSeniorities: ["mid", "senior"], requiredSkills: ["SQL", "Python"], preferredSkills: [], excludedSkills: [], acceptedWorkModels: ["remote", "hybrid"], locations: ["Brazil"] },
];

const companies: TargetCompany[] = [
  { id: "11111111-1111-4111-8111-111111111111", name: "Example Platform", provider: "greenhouse", boardIdentifier: "example-platform", enabled: true, priority: "high", createdAt: "2026-01-01T00:00:00Z", updatedAt: "2026-01-01T00:00:00Z" },
  { id: "22222222-2222-4222-8222-222222222222", name: "Sample Studio", provider: "lever", boardIdentifier: "sample-studio", enabled: false, priority: "normal", createdAt: "2026-01-01T00:00:00Z", updatedAt: "2026-01-01T00:00:00Z" },
];
...
describe("Dashboard", () => {
  ...
  it("supports a single persisted profile and an empty profile list", () => {
    const { rerender } = render(<Dashboard profiles={[profiles[0]]} jobs={jobs} analyses={jobAnalyses} />);
    ...
    expect(screen.getByRole("heading", { name: "Create your first candidate profile" })).toBeInTheDocument();
  });

  it("summarizes configured companies without implying collection is active", () => {
    render(<Dashboard profiles={profiles} jobs={jobs} analyses={jobAnalyses} companies={companies} />);
    expect(screen.getByRole("heading", { name: "Company monitoring" })).toBeInTheDocument();
    expect(screen.getByText("Configuration only — job collection is not active yet.")).toBeInTheDocument();
