      ...
      <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-4 flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
            <div>
              <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">
                Job intelligence for focused decisions.
              </h1>
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
                Review a tailored view of opportunities and the reasoning behind each
                recommendation.
              </p>
            </div>
            <div className="w-full sm:w-72">
              <label className="mb-2 block text-sm font-medium" htmlFor="candidate-profile">
                Candidate profile
              </label>
              <select
                ...
                className="w-full rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm shadow-sm outline-none focus:border-blue-700 focus:ring-2 focus:ring-blue-200"
              >
        ...
        <section className="mb-8 rounded-lg border border-slate-200 bg-white p-5">
          <div className="flex justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold">Collected jobs</h2>
              <p className="mt-1 text-sm text-slate-600">
                {persistedJobCount} stored source jobs. No AI analysis has been performed.
              </p>
            </div>
            <Link
              href="/jobs"
        ...
        >
          ...
          {jobStatusCountsError ? (
            ...
            <dl className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <Metric label="Saved" value={statusCounts.saved} tone="text-blue-800" />
              <Metric label="Applied" value={statusCounts.applied} tone="text-emerald-800" />
              <Metric label="Ignored" value={statusCounts.ignored} tone="text-slate-600" />
              <Metric label="Rejected" value={statusCounts.rejected} tone="text-red-800" />
            </dl>
          )}
        </section>
        <section className="mb-8 rounded-lg border border-slate-200 bg-white p-5">
          <h2 className="text-lg font-semibold">Deterministic filtering</h2>
          <p className="mt-1 text-sm text-slate-600">
            Evaluate collected jobs with explicit rule-based profile preferences — no AI.
          </p>
          <Link
            href="/jobs/evaluate"
        ...
        >
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <div>
              <h2 id="company-summary-heading" className="text-lg font-semibold">
                Company monitoring
              </h2>
              <p className="mt-1 text-sm text-slate-600">
                Configuration only — job collection is not active yet.
              </p>
            </div>
            <Link
              href="/companies"
          ...
          {companyError ? (
            ...
            <dl className="mt-4 grid gap-3 sm:grid-cols-3">
              <Metric label="Monitored companies" value={companies.length} />
              <Metric
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4 text-sm">
        ...
        {job.originalUrl ? (
          <a
            className="font-medium text-blue-700 underline underline-offset-4 focus:outline-none focus:ring-2 focus:ring-blue-300"
...
function InsightList({
  ...
  return (
    <div>
      ...
      <ul className={`mt-2 space-y-1 text-sm ${muted ? 'text-slate-600' : 'text-slate-700'}`}>
        {items.map((item) => (
