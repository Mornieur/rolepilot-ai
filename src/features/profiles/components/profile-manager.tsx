...
function ProfileCard({
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <details>
        <summary className="cursor-pointer text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-blue-300">
          {profile.name}
        </summary>
        <div className="mt-5">
          <ProfileForm
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="mt-2 rounded-md border border-red-300 px-3 py-2 text-sm font-semibold text-red-800 focus:outline-none focus:ring-2 focus:ring-red-200 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400"
      >
        {pending ? 'Deleting…' : 'Delete profile'}
      </button>
      {disabled && (
        <p className="mt-2 text-sm text-slate-500">At least one candidate profile must remain.</p>
      )}
    </form>
  );
}
