...
import { toCandidateProfile, toCandidateProfileInsert } from "@/features/profiles/server/profile-mapper";
...
export class CandidateProfileDataError extends Error {
  constructor(message = "Candidate profiles are unavailable right now. Please try again.") { super(message); }
}
...
export async function listCandidateProfiles(): Promise<CandidateProfile[]> {
  const { data, error } = await getSupabaseServerClient().from("candidate_profiles").select("*").order("created_at");
...
export async function getCandidateProfileById(id: string): Promise<CandidateProfile | null> {
  const { data, error } = await getSupabaseServerClient().from("candidate_profiles").select("*").eq("id", id).maybeSingle();
...
export async function createCandidateProfile(input: CandidateProfileInput): Promise<CandidateProfile> {
  const { data, error } = await getSupabaseServerClient().from("candidate_profiles").insert(toCandidateProfileInsert(input)).select().single();
  if (error || !data) throwDataError("create");
  return toCandidateProfile(data);
}

export async function updateCandidateProfile(id: string, input: CandidateProfileInput): Promise<CandidateProfile> {
  const { data, error } = await getSupabaseServerClient().from("candidate_profiles").update(toCandidateProfileInsert(input)).eq("id", id).select().single();
...
export async function deleteCandidateProfile(id: string): Promise<void> {
  ...
  const { count, error: countError } = await client.from("candidate_profiles").select("id", { count: "exact", head: true });
  if (countError || count === null) throwDataError("count");
  if (count <= 1) throw new CandidateProfileDataError("The last candidate profile cannot be deleted.");
